  ...
  if (a){
    ...
    for (i = 0; i <= a->nchaves - 1; i++){
      ...
      if ((a->active[i] == 0 && isAtivo == 0) || (a->active[i] != 0 && isAtivo != 0)){
        // if(a->active == 0){
        if (a->slans != 0){
          l = TLSEp_insere(l, pointCalc(a->slans[i]), PL_criaPL(a, i)); // cria um novo elemento pra l com o calculo da pontua��o e um struct com referencias para o no original
          // TLSEp_imprime(l);
        }
      }
    }
  ...
  for (int i = 0; i < a->nchaves; i++){
    if (a->active[i]){
      if (strcmp(a->active[i]->nasc, ch) == 0){
        ...
        i = 0;                            // repassa a arvore para evitar que haja qualquer jogador de tal nacionalidade que tenha sido movido por alguma rota��o
      }
    }
  }
  for (int i = 0; i <= a->nchaves; i++)
